...
func _init() -> void:
	...
	# --- Array / JSON text edit (hidden in dictionary mode) ---
	...
	# --- Dictionary row editor (hidden in array mode) ---
...
func setup(
	...
	# Show the right layout for this mode
	_text_edit.visible = not _is_dictionary
...
func focus_main() -> void:
	if _is_dictionary:
		# Focus the first key field if rows exist
		if _dict_rows.get_child_count() > 0:
			var first_row := _dict_rows.get_child(0) as HBoxContainer
			if first_row != null and first_row.get_child_count() > 0:
				var key_edit := first_row.get_child(0) as LineEdit
				if key_edit != null:
					key_edit.grab_focus()
					return
...
func _rebuild_dict_rows(dict: Dictionary) -> void:
	# Remove existing rows without touching other children of _dict_rows
...
func _append_dict_row(key_text: String, value_text: String) -> void:
	...
	val_edit.custom_minimum_size = Vector2(100, 0)
...
func _parse_dict_rows() -> Variant:
	...
	for row_node in _dict_rows.get_children():
		...
		if row == null:
			continue
		# Children: key_edit (0), sep label (1), val_edit (2), remove_btn (3)
		if row.get_child_count() < 4:
			continue
		var key_edit := row.get_child(0) as LineEdit
